...
function convert_11_2_100_to_ethiopic(num: number) {
    if (num === 100) {
        ...
        return result[1] > 0 ? `${ethiopicNumber[result[0] * 10]}${ethiopicNumber[result[1]]}` : `${ethiopicNumber[result[0] * 10]}`
    }
}
...
function convert_to_ethiopic(num: number) {
    ...
    if (num > 1000 && num <= 10000) {
        const result = divide(100, num)
        if (result[1] === 0) {
            return result[0] < 11 ? `${ethiopicNumber[result[0]]}${ethiopicNumber[100]}` : `${convert_11_2_100_to_ethiopic(result[0])}${ethiopicNumber[100]}`
        }
