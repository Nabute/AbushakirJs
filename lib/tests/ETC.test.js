...
describe('Testing the `yearDays` method', function () {
    ...
    it('yearDays method returns an array of days for the entire year', function () {
        ...
        yearDays.forEach(function (monthDays, index) {
            ...
            var expectedDaysInMonth = etc["_monthRange"]()[1];
            var expectedFirstDayOfWeek = etc["_monthRange"]()[0];
...
describe('Testing the `toNumber` method', function () {
    it('Converts various input types to numbers', function () {
        var etc = new etc_1.default(2011, 11, 14);
        ...
        expect(function () { return etc['toNumber'](Symbol('test')); }).toThrowError('TYPE ERROR: Unexpected operand type.');
        // Object should throw an error
        expect(function () { return etc['toNumber']({}); }).toThrowError('TYPE ERROR: Unexpected operand type.');
        // Other number types should be returned as is
        expect(etc['toNumber'](42)).toBe(42);
        expect(etc['toNumber'](3.14)).toBe(3.14);
    });
});
describe('Testing the `_monthRange` method', function () {
    var etc;
    beforeEach(function () {
        etc = new etc_1.default(2023, 11, 14);
    });
    it('Valid month range', function () {
        var monthRange = etc['_monthRange']();
        expect(monthRange).toEqual([1, 30]);
    });
    it.skip('Invalid month number', function () {
        etc = new etc_1.default(2011, 15, 14);
        expect(function () { return etc['_monthRange'](); }).toThrowError('MONTHNUMBER ERROR: Month number should be between 1 and 13.');
    });
});
