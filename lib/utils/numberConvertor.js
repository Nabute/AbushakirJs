...
function convert_101_2_1000_to_ethiopic(num) {
    ...
    const left = result[0] === 1 ? `${ethiopicNumber[100]}` : `${ethiopicNumber[result[0]]}${ethiopicNumber[100]}`;
    const right = result[1] <= 10 ? convert_1_2_10_to_ethiopic(result[1]) : convert_11_2_100_to_ethiopic(result[1]);
    return `${left}${right}`;
}
